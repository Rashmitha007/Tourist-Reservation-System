...
char adminf[20]="Trippp.txt";
char userf[20]="Reservationn.txt";
...
int binarySearch(tour arr[], int left, int right, char searchCode[]) {
    while (left <= right) {
        int mid = left + (right - left) / 2;

        // Check if the search code is found at the middle position
        if (strcmp(arr[mid].ad_code, searchCode) == 0)
            return mid;

        // If the search code is greater, ignore the left half
        if (strcmp(arr[mid].ad_code, searchCode) < 0)
            left = mid + 1;

        // If the search code is smaller, ignore the right half
        else
            right = mid - 1;
    }

    // Search code is not found
    return -1;
}

void Tourist::ad_search() {
    fstream fp;
    char ad_code[15], buff[45];
    tour r;
    cout << "\nEnter the Destination Code to be searched: D-";
    cin >> ad_code;
    fp.open(adminf, ios::in);
    if (!fp) {
        cout << "\nCannot open file";
    ...
    while (fp.getline(buff, 45)) {
        r = ad_unpack(buff);
        if (strcmp(r.ad_code, ad_code) == 0) {
            cout << "\nTRIP FOUND\n";
            cout << "Destination Code: " << r.ad_code << endl;
            cout << "Destination Place: " << r.ad_place << endl;
            cout << "Destination Country: " << r.ad_country << endl;
            cout << "No of days of Trip: " << r.ad_days << endl;
            cout << "Total Cost of the Trip: " << r.ad_prices << endl;
            found = true;
            break;
        }
    }
...
void Tourist::transfer_reservation() {
    fstream fp;
    char old_name[15], new_name[15], buff[45];
    int i, j;
    tour s[100];
    fp.open(userf, ios::in);
    if (!fp) {
    ...
    cout << "\nEnter the name of the person whose reservation is to be transferred: ";
    cin >> old_name;
    cout << "Enter the name of the new person: ";
    cin >> new_name;
    ...
    for (j = 0; j < i; j++) {
        if (strcmp(s[j].us_name, old_name) == 0) {
            strcpy(s[j].us_name, new_name);
            break;
        }
    }
    ...
    fd.close();
    cout << "\nReservation transferred successfully from " << old_name << " to " << new_name << endl;
}
